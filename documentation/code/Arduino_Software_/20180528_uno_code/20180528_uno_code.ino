...
const int DistanceThreshold=70;//default 150
...
// get value from distance sensor and activate
unsigned long reward_fun(uint16_t value,long unsigned LastTime){ 
 //Serial.println("enter sensor read");
 if (value>DistanceThreshold){//IR tripped
  //Serial.println("IR tripped");
    unsigned long currentTime=millis();    
    Serial.println((currentTime/1000));     
    Serial.println(iwait/1000);
    Serial.println();
   unsigned long now=(LastTime+interval);
   if ((long) (millis()-iwait)>=0){
    iwait+=interval;
   
//    if ((currentTime+interval)>LastTime) {        
     ...
     waitXmillisec(1000);    
     unsigned long LastTime=currentTime;     //reset last time activated
     return LastTime; // return last time
    }
    else{
      //Serial.println("DO NOT reward");
      unsigned long LastTime=LastTime;     //reset last time activated
      return LastTime; } // return last time    Last_time=pre
...
////////////////start setup//////////////////////////////
void setup(){ 
  iwait=millis()+interval;
  unsigned long LastTime =millis();//(millis()-interval);// counter 
...
//sensor 1 is ON and 2,3 OFF & this arm is selected
  if (ArmState== HIGH && Sensor1State == HIGH && Sensor2State == LOW && Sensor3State == LOW){
   ...
   LastTime=reward_fun(value,LastTime);
   }
